...
vector<vector<string>> solve(std::filesystem::path const& path, std::string_view valid_chars) {
    ...
    auto const firstChar = [&dictionary](size_t index) { return dictionary[index + 1]; };
    auto const lastChar = [&dictionary](size_t index) { return dictionary[index + dictionary[index]]; };
    auto const wordSize = [&dictionary](size_t index) { return dictionary[index]; };
    ...
    auto const modify = [wordSize, &dictionary]<class T>(std::span<uint8_t, 26> counter, size_t row_begin, T) {
        ...
        auto const rowBegin = row_begin + 1;
        auto const rowEnd = rowBegin + wordSize(row_begin);
